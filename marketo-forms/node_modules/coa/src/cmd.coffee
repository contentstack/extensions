    ...
    _parseCmd: (argv, unparsed = []) ->
        ...
        while i = argv.shift()
            ...
            if not optSeen and /^\w[\w-_]*$/.test(i)
                ...
                if not cmd and @_ext
                    ***REMOVED***construct package name to require
                    if typeof @_ext is 'string'
                        if ~@_ext.indexOf('%s')
                            ***REMOVED***use formatted string
                            pkg = UTIL.format(@_ext, i)
                        else
                            ***REMOVED***just append subcommand name to the prefix
                            pkg = @_ext + i
                    else if @_ext is true
                        ***REMOVED***use default scheme: <command>-<subcommand>-<subcommand> and so on
                        pkg = i
                        c = @
                        loop
                            pkg = c._name + '-' + pkg
                    ...
                    if cmdDesc
                        if typeof cmdDesc == 'function'
                            ***REMOVED***set create subcommand, set its name and apply imported function
                            @cmd()
                                .name(i)
                                .apply(cmdDesc)
                                .end()
                        else if typeof cmdDesc == 'object'
                            ***REMOVED***register subcommand
                            @cmd(cmdDesc)
                            ***REMOVED***set command name
                            cmdDesc.name(i)
                        else
            ...
            ***REMOVED***opt
            if i isnt '--' and not i.indexOf '-'
                ...
                if m = i.match /^(--\w[\w-_]*)=(.*)$/
                    ...
                    ***REMOVED***suppress 'unknown argument' error for flag options with values
            ...
            Q.all([@_processParams(opts, @_opts), @_processParams(args, @_args)])
                .spread (opts, args) =>
                    @_do({ cmd: p.cmd, opts: opts, args: args })
                        ***REMOVED***catch fails from .only() options
                        .fail (res) =>
