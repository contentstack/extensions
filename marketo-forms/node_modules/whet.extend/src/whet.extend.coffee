...
_isPlainObj = (obj) ->
  ...
  ***REMOVED***Not own constructor property must be Object
  return false if obj.constructor and
                  not _isOwnProp(obj, 'constructor') and
                  not _isOwnProp(obj.constructor::, 'isPrototypeOf')

  ***REMOVED***Own properties are enumerated firstly, so to speed up, 
  ***REMOVED***if last one is own, then all properties are own.
  key for key of obj
...
_findValue = (deep, copy, src) ->
  ***REMOVED***if we're merging plain objects or arrays
