...
gulp.task('default', () => {
    return gulp.src('./build/*.html')
        ...
        // Remove unnecessary references for Contentstack extensions (not needed for inline HTML)
        .pipe(replace(/<link[^>]*rel="manifest"[^>]*>/g, ''))
        .pipe(replace(/<link[^>]*rel="apple-touch-icon"[^>]*>/g, ''))
