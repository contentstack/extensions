...
declare global {
    interface Window { ContentstackUIExtension: any; }
}
    ...
    useEffect(() => {
        ...
        ContentstackUIExtension.init().then((extensions: ExtensionObj) => {
            ...
            if (items && typeof items[0] !== "object") {
                ...
                brightcove.getVidoes(data).then((video) => {
                    setVideoList(video || []);
                })
            } else {
                setVideoList(items);
                                ...
                                {videoList && videoList.map((video) => {
                                    return (
                                        ...
                                        >
                                            <div className="file">
                                                <a
                                                    href={video.images.poster.source ?
                                                        video.images.poster.source[0].src :
                                                        video.images.poster.src
                                                    }
                                                    target="_blank"
                                                    className="fileimage" rel="noreferrer"
                                                >
                                                    <span
                                                        className="fileimg"
                                                        style={{
                                                            backgroundImage: `url(${video.images.thumbnail.source ?
                                                                video.images.poster.source[0].src :
                                                                video.images.poster.src})`,
                                                        }}
                                                    ></span>
